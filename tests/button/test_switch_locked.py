...
from components.button import SwitchLocked


def test_locked_default_true():
    line_edit = QLineEdit()
    switch_locked_radio_button = SwitchLocked(effect_group=[line_edit])
...
def test_locked_default_false():
    line_edit = QLineEdit()
    switch_locked_radio_button = SwitchLocked(locked_default=False, effect_group=[line_edit])
